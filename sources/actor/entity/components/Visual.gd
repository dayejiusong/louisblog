...
func SetSkinSlot(slot : ActorCommons.Slot, raceData : RaceData, textures : Array):
	...
	if not entity.stat.IsMorph():
		...
		if not hasOverrideTexture:
			var texturePath : String = textures[entity.stat.gender] if entity.stat.gender >= 0 and entity.stat.gender < textures.size() else ""
			if not texturePath.is_empty():
				slotTexture = FileSystem.LoadGfx(texturePath)
