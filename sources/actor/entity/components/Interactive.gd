...
func DisplaySpeech(speech : String):
	...
	if speechContainer:
		...
		Localization.DisableAutoTranslate(speechLabel)
...
func Init(data : EntityData):
	...
	if nameLabel:
		Localization.DisableAutoTranslate(nameLabel)
		nameLabel.set_text(entity.nick if entity.type == ActorCommons.Type.PLAYER else Localization.Text(entity.nick))
