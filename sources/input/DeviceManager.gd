...
static func GetActionName(action : String) -> String:
	return Localization.Text(actionNames[action]) if HasActionName(action) else ""
...
static func GetJoyButtonName(buttonIndex : int) -> String:
	if buttonIndex >= 0 and buttonIndex < joyButton.size():
		return Localization.Text(joyButton[buttonIndex])
	return str(buttonIndex)

static func GetMouseButtonName(buttonIndex : MouseButton) -> String:
	match buttonIndex:
		MOUSE_BUTTON_LEFT:		return Localization.Text("Mouse Left")
		MOUSE_BUTTON_RIGHT:		return Localization.Text("Mouse Right")
		MOUSE_BUTTON_MIDDLE:	return Localization.Text("Mouse Middle")
		MOUSE_BUTTON_WHEEL_UP:	return Localization.Text("Mouse Wheel Up")
		MOUSE_BUTTON_WHEEL_DOWN:return Localization.Text("Mouse Wheel Down")
		_:						return Localization.Format("Mouse %d", [buttonIndex])

static func GetJoyAxisName(axis : JoyAxis, value : float) -> String:
	var axisName : String = ""
	match axis:
		JOY_AXIS_LEFT_X:		axisName = Localization.Text("Left Stick X")
		JOY_AXIS_LEFT_Y:		axisName = Localization.Text("Left Stick Y")
		JOY_AXIS_RIGHT_X:		axisName = Localization.Text("Right Stick X")
		JOY_AXIS_RIGHT_Y:		axisName = Localization.Text("Right Stick Y")
		JOY_AXIS_TRIGGER_LEFT:	axisName = Localization.Text("Left Trigger")
		JOY_AXIS_TRIGGER_RIGHT:	axisName = Localization.Text("Right Trigger")
		_:						axisName = Localization.Format("Axis %d", [axis])
	if value > 0:
		axisName += "+"
	elif value < 0:
		axisName += "-"
