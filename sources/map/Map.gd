...
func SpawnEntity(agentRID : int, entry : EntityCacheEntry) -> Entity:
	...
	var expectedLocalPlayerName : String = Launcher.LocalPlayerName
	if expectedLocalPlayerName.is_empty() and Launcher.GUI and Launcher.GUI.characterPanel:
		expectedLocalPlayerName = Launcher.GUI.characterPanel.characterNameDisplay.get_text()
	var isLocalPlayer : bool = isPlayerType and entry.nick == expectedLocalPlayerName
