...
func _GetFirstID(entries : Dictionary) -> int:
	for entryID in entries:
		return int(entryID)
	return DB.UnknownHash

func _BuildSinglePlayerTraits() -> Dictionary:
	var traits : Dictionary = ActorCommons.DefaultTraits.duplicate(true)
	var raceID : int = _GetFirstID(DB.RacesDB)
	var hairstyleID : int = _GetFirstID(DB.HairstylesDB)
	var haircolorID : int = _GetFirstID(DB.PalettesDB[DB.Palette.HAIR])
	var skintoneID : int = DB.UnknownHash
	var raceData : RaceData = DB.RacesDB.get(raceID, null)

	if raceData:
		skintoneID = _GetFirstID(raceData._skins)

	traits["gender"] = ActorCommons.Gender.MALE
	traits["race"] = raceID
	traits["hairstyle"] = hairstyleID
	traits["haircolor"] = haircolorID
	traits["skintone"] = skintoneID
	return traits

func _EnsureSinglePlayerCharacter(accountID : int, nickname : String) -> NetworkCommons.CharacterError:
	var characterID : int = Launcher.SQL.GetCharacterID(accountID, nickname)
	if characterID != NetworkCommons.PeerUnknownID:
		return NetworkCommons.CharacterError.ERR_OK

	var traits : Dictionary = _BuildSinglePlayerTraits()
	var attributes : Dictionary = ActorCommons.DefaultAttributes.duplicate(true)
	traits.merge(ActorCommons.DefaultTraits)

	if not ActorCommons.CheckTraits(traits) or not ActorCommons.CheckAttributes(attributes):
		return NetworkCommons.CharacterError.ERR_MISSING_PARAMS
	if not Launcher.SQL.AddCharacter(accountID, nickname, ActorCommons.DefaultStats, traits, attributes):
		return NetworkCommons.CharacterError.ERR_NAME_AVAILABLE

	characterID = Launcher.SQL.GetCharacterID(accountID, nickname)
	if characterID == NetworkCommons.PeerUnknownID:
		return NetworkCommons.CharacterError.ERR_NO_CHARACTER_ID
...
func BootstrapSinglePlayer(peerID : int):
	var err : NetworkCommons.AuthError = NetworkCommons.AuthError.ERR_OK
	var peer : Peers.Peer = Peers.GetPeer(peerID)
	if not peer:
		err = NetworkCommons.AuthError.ERR_NO_PEER_DATA
	else:
		var accountName : String = LauncherCommons.WebSinglePlayerAccountName
		var password : String = LauncherCommons.WebSinglePlayerPassword
		var characterName : String = LauncherCommons.WebSinglePlayerCharacterName

		if not Launcher.SQL.HasAccount(accountName) and not Launcher.SQL.AddAccount(accountName, password, ""):
			err = NetworkCommons.AuthError.ERR_NAME_AVAILABLE
		...
		if err == NetworkCommons.AuthError.ERR_OK:
			...
			if not accountData:
				err = NetworkCommons.AuthError.ERR_AUTH
			elif peer.accountID != accountData.accountID:
				err = Peers.FinalizeLogin(peer, accountName, accountData)

		if err == NetworkCommons.AuthError.ERR_OK:
			var createErr : NetworkCommons.CharacterError = _EnsureSinglePlayerCharacter(peer.accountID, characterName)
			if createErr != NetworkCommons.CharacterError.ERR_OK:
				err = NetworkCommons.AuthError.ERR_SERVER_UNREACHABLE

		if err == NetworkCommons.AuthError.ERR_OK:
			var characterID : int = Launcher.SQL.GetCharacterID(peer.accountID, characterName)
			if peer.characterID != characterID:
				var connectErr : NetworkCommons.CharacterError = _ConnectCharacter(characterName, peerID)
				if connectErr != NetworkCommons.CharacterError.ERR_OK:
					err = NetworkCommons.AuthError.ERR_SERVER_UNREACHABLE
...
func _ConnectCharacter(nickname : String, peerID : int) -> NetworkCommons.CharacterError:
	...
	if not peer:
		...
		var characterID : int = Launcher.SQL.GetCharacterID(peer.accountID, nickname)
		if Peers.GetCharacter(peerID) != NetworkCommons.PeerUnknownID:
			if peer.characterID == characterID:
				return NetworkCommons.CharacterError.ERR_OK
			err = NetworkCommons.CharacterError.ERR_ALREADY_LOGGED_IN
		else:
			peer.SetCharacter(characterID)
			if peer.characterID == NetworkCommons.PeerUnknownID:
				err = NetworkCommons.CharacterError.ERR_NO_CHARACTER_ID
			else:
...
func ConnectCharacter(nickname : String, peerID : int):
	var err : NetworkCommons.CharacterError = _ConnectCharacter(nickname, peerID)
	Network.CharacterError(err, peerID)
	return err
