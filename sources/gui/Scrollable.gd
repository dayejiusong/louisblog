	...
	if "categories" in dictionary:
		for category in dictionary["categories"]:
			if "category" in category:
				...
				label.text = "[center][color=#" + UICommons.LightTextColor.to_html(false) + "]" + Localization.Text(category["category"]) + "[/color][/center]\n"
...
func AddTitle(entry : Dictionary):
	if "title" in entry:
		...
		Localization.DisableAutoTranslate(label)
		label.text = "[color=#" + UICommons.LightTextColor.to_html(false) + "]" + Localization.Text(entry["title"])
		if "date" in entry:
...
func AddContent(entry : Dictionary):
	if "content" in entry:
		...
		Localization.DisableAutoTranslate(label)
		label.text = "[color=#" + UICommons.TextColor.to_html(false) + "]" + Localization.Text(entry["content"]) + "[/color]\n"
		textContainer.add_child.call_deferred(label)
