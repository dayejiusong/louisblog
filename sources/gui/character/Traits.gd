...
func RefreshHairstyle():
	...
	if hairstyleValue >= 0 and hairstyleValue < hairstylesCount:
		hairstyleLabel.set_text(Localization.Text(DB.GetHairstyle(hairstyles[hairstyleValue])._name))
		hairUpdate.emit()
...
func RefreshHaircolor():
	...
	if haircolorValue >= 0 and haircolorValue < haircolorsCount:
		haircolorLabel.set_text(Localization.Text(DB.GetPalette(DB.Palette.HAIR, palettes[haircolorValue])._name))
		hairUpdate.emit()
...
func RefreshRace():
	...
	if raceValue >= 0 and raceValue < raceCount:
		...
		raceLabel.set_text(Localization.Text(data._name))
		RefreshSkintone()
		bodyUpdate.emit()
...
func RefreshSkintone():
	...
	if raceValue >= 0 and raceValue < raceCount:
		...
		if skintoneValue >= 0 and skintoneValue < skintoneCount:
			skintoneLabel.set_text(Localization.Text(skins[skinsKeys[skintoneValue]]._name))
			bodyUpdate.emit()
