...
func FillWarningLabel(err : NetworkCommons.CharacterError):
	...
	match err:
		...
		NetworkCommons.CharacterError.ERR_ALREADY_LOGGED_IN:
			warn = Localization.Text("Character is already logged in.")
		NetworkCommons.CharacterError.ERR_TIMEOUT:
			warn = Localization.Format("Could not connect to the server (Error %d).", [err])
		NetworkCommons.CharacterError.ERR_MISSING_PARAMS:
			warn = Localization.Text("Some character information are missing.")
		NetworkCommons.CharacterError.ERR_NAME_AVAILABLE:
			warn = Localization.Text("Character name not available.")
...
func HasSelectedCharacter() -> bool:
	return HasSlot(currentCharacterID) and not charactersInfo[currentCharacterID].is_empty() and not characterNameDisplay.get_text().is_empty()
...
func SelectCharacter():
	if not HasSelectedCharacter():
		return

	Launcher.LocalPlayerName = characterNameDisplay.get_text()
...
func RefreshButtons():
	...
	var hasFreeSlot : bool = NextAvailableSlot() != ActorCommons.InvalidCharacterSlot
	if HasSelectedCharacter():
		...
		Launcher.GUI.buttonBoxes.Bind(UICommons.ButtonBox.PRIMARY, "New Player", EnableCharacterCreator.bind(true))
...
func _ready():
	...
	Localization.DisableAutoTranslate(characterNameLineEdit)
	Localization.DisableAutoTranslate(characterNameDisplay)
