...
func GetTooltipWeight() -> String:
	if cell.weight == 0:
		return ""
	var lightColor : String = "#" + UICommons.LightTextColor.to_html(false)
	return "\n\n%s: [color=%s]%dg[/color]" % [Localization.Text("Weight"), lightColor, cell.weight]

func GetTooltipHeader() -> String:
	var bbcode : String = Localization.Text(cell.name)
	if cell is ItemCell and not cell.customfield.is_empty():
		bbcode += " (%s)" % Localization.Text(cell.customfield)
	if cell.description:
		bbcode += "\n%s" % Localization.Text(cell.description)
	if cell is SkillCell and Launcher.Player and Launcher.Player.progress:
		bbcode += "\n" + Localization.Format("Level: %d", [Launcher.Player.progress.GetSkillLevel(cell)])
	return bbcode
