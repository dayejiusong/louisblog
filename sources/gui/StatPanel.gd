...
func RefreshVitalStats():
	...
	lLevel.set_text(Localization.Format("Lv. %d", [panelStats.level]))
	var spiritData : EntityData = DB.EntitiesDB.get(panelStats.spirit, null)
	if spiritData:
		lSpirit.set_text(Localization.Text(spiritData._name))
	...
	lGP.set_text(Localization.Format("%s GP", [Util.GetFormatedText(str(panelStats.gp))]))
...
func _ready():
	Localization.DisableAutoTranslate(lName)
	_post_launch()
